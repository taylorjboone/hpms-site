...
mypath='C:\PythonTest\Voltron\district_chrystal_report_website\hpms-site\static\geo_count_conversion'
number=1
...
def metro_count_convert(in_filename,out_filename='default.xlsx'):
    ...
    z['Wheel Base']=z[5]
    z[['Quality Control','b']]=z[6].str.split(';',expand=True)
    z=z.drop([0,1,2,3,4,5,6,'-a','b'],axis=1)
    z['CountType']='Metro'
...
def centurion_count_convert(in_filename,out_filename='default.xlsx'):
    ...
    z['CountType']='Centurion'

    return z.to_excel(out_filename,index=False)


def geo_count_process(x,cent_number,metro_number):
    ...
    # for a in onlyfiles:
        # print(a)
    file=open(x).read().splitlines()
    is_cent = 'Centurion' in file[3] and 'FV' in file[3]
    is_metro = 'MetroCount' in file[3] and 'FV' in file[3]

    if is_cent==True:
        new_name=r'%s_%d.xlsx' % (x, cent_number)
        cent_number+=1
        centurion_count_convert(x,out_filename=new_name)
    if is_metro==True:
        new_name=r'%s_%d.xlsx' % (x, metro_number)
        metro_number+=1
        metro_count_convert(x,out_filename=new_name)

    return (metro_number,cent_number,new_name)
